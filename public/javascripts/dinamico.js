...
function eliminar(id) {
    if (confirm('Esta Seguro?')) {
        ...
        toastr.warning("El inventario se ha Eliminado!.", "Importante!");
        $.ajax({
            url: "/" + $("#modulo").attr("class") + "/",
            type: 'DELETE',
            data: {
                'id': id
            },
...
function generarCampos() {
    ...
    $.ajax({
        url: "/formulario/" + $("#modulo").attr("class") + "/",
        type: 'POST',
        data: {
    ...
    $("#guardar").click(function() {
        ...
        $.ajax({
            ...
            data: {
                ...
                'modulo': $("#modulo").attr("class"),
                'establecimiento': $("#establecimiento").attr("class")
            },
            error: function(respuesta) {
                console.log(respuesta);
            },
                ...
                action: function(e, dt, node, conf) {
                    if (confirm('Esta Seguro?')) {
                        ...
                        $("input[type='checkbox']").each(function() {
                            if ($(this).is(":checked")) {
                                $.ajax({
                                    url: "http://" + window.location.hostname + ":" + window.location.port + "/" + $("#modulo").attr("class") + "/",
                                    type: 'DELETE',
                                    data: {
                                        'id': $(this).attr("class")
                                    },
