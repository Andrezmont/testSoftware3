(function() {
    ...
    exports.cuestionario = function(req, res) {
        req.app.db.models.cuestionario.find({
            'data.valor': req.body.area
        }, {
            'data': 1
        }).exec(function(err, ask) {
            if (err) console.log(err);
            var ars = [];
            var respuesta, od, oc, ob, oa, ra, rb, rc, rd, encabezado, relacion, indice;
    ...
    exports.historial = function(req, res) {
        req.app.db.models.User.findById(req.user.id, {
            ...
            req.app.db.models.registro.find({
                'empresa': user.empresa
            }).sort({
                "fecha_creacion": -1
            }).exec(function(err, ask) {
                if (err) console.log(err);
                res.render('grafica/historial', {
                    'historial': ask
                });
            });
        });
