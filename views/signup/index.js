(function() {
  ...
  exports.init = function(req, res){
    req.app.db.models.empresa.find({},function(err, doc) {
      var salida = [];
      for (var i = 0; i < doc.length; i++) {
        salida[i]= {'Nombre':doc[i].data[0].valor,'id':doc[i]._id};
      }
  ...
  exports.signup = function(req, res){
    ...
    workflow.on('validate', function() {
      if (!req.body.username) {
        workflow.outcome.errfor.username = 'Usuario Requerido!';
      }
      if (!req.body.empresa) {
        workflow.outcome.errfor.empresa = 'Empresa Requerido!';
      }
      else if (!/^[a-zA-Z0-9\-\_]+$/.test(req.body.username)) {
        workflow.outcome.errfor.username = 'Solo letras, numeros, \'-\', \'_\'';
      }

      if (!req.body.email) {
        workflow.outcome.errfor.email = 'Correo Requerido!';
      }
      else if (!/^[a-zA-Z0-9\-\_\.\+]+@[a-zA-Z0-9\-\_\.]+\.[a-zA-Z0-9\-\_]+$/.test(req.body.email)) {
        workflow.outcome.errfor.email = 'Formato de Correo Invalido!';
      }

      if (!req.body.password) {
        workflow.outcome.errfor.password = 'Contraseña Requerida!';
      }
    ...
    workflow.on('duplicateUsernameCheck', function() {
      req.app.db.models.User.findOne({ username: req.body.username }, function(err, user) {
        ...
        if (user) {
          workflow.outcome.errfor.username = 'Usuario ya Existe!';
          return workflow.emit('response');
        }
    ...
    workflow.on('duplicateEmailCheck', function() {
      req.app.db.models.User.findOne({ email: req.body.email.toLowerCase() }, function(err, user) {
        ...
        if (user) {
          workflow.outcome.errfor.email = 'Correo ya Existe!';
          return workflow.emit('response');
        }
